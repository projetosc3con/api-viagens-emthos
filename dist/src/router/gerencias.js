...
const collection = firebase_1.admin.firestore().collection('GERENCIAS');
// Criar gerencia
...
router.get('/', async (req, res) => {
    try {
        ...
        const gerencias = snapshot.docs.map(doc => ({
            id: doc.id,
            ...doc.data(),
        }));
        res.json(gerencias);
...
router.get('/:id', authenticate_1.authenticate, async (req, res) => {
    ...
    try {
        ...
        if (!doc.exists)
            return res.status(404).json({ error: 'gerencia não encontrada' });
